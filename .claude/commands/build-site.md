...
> **设计规范：** 本 skill 的所有设计决策必须遵循 `.claude/commands/design-system.md` 文档。
> 开始前先用 Read 工具读取该文件，将其中的视觉规范、动效标准、反模式清单内化为执行标准。
...
> 从 `design-system.md` 的「一、设计风格预设」中选择最匹配的 Vibe（A/B/C/D/E），
> 以该 Vibe 的色值、字重、层次感规则为基准生成方案。
...
> 生成每个文件时，严格对照 `design-system.md` 执行：
> - **视觉**：三层背景结构（光晕 / 纹理 / 内容）、颜色层次、边框规范
> - **排版**：字号比例、字重对比、行距、负字间距大标题
> - **动效**：从「五、动效规范」选择匹配场景的模式，贝塞尔曲线用标准值
> - **微交互**：每个交互元素覆盖「六、微交互清单」的全部状态
> - **禁令**：「八、反模式清单」逐条检查，一条都不能触碰
